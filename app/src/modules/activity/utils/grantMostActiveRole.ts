...
export const grantMostActiveRole = async (
	...
	var activeUserVoice = await client.db.repos.activityVoice.findOne({
		where: { guildId: guildId },
		order: { dailySeconds: 'DESC' },
	});
	...
	var mostActiveRole;
	if (type === 'daily')
		mostActiveRole = guild.roles.cache.get(actConf.dailyActiveRole);
	if (type === 'weekly')
		mostActiveRole = guild.roles.cache.get(actConf.weeklyActiveRole);

	if (!mostActiveRole) return;
	mostActiveRole.members.forEach(
		async (member) => await member.roles.remove(mostActiveRole).catch()
	);
