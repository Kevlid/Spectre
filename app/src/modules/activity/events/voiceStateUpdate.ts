...
export const VoiceStateUpdate: Event = {
	...
	async execute(client: KiwiClient, oldVoiceState: VoiceState, newVoiceState: VoiceState) {
		...
		var userVoiceState = await getVoiceState(client, newVoiceState.guild.id, newVoiceState.id);

		if (userVoiceState && !newVoiceState.channelId) {
			// User left voice channel
			await removeVoiceState(client, newVoiceState.guild.id, newVoiceState.id);

			var secondsSinceLastUpdate =
				(new Date().getTime() - userVoiceState.lastUpdate.getTime()) / 1000;
			await saveVoice(
				client,
			...
			await createVoiceState(client, newVoiceState.guild.id, newVoiceState.id);
		}
	},
};
