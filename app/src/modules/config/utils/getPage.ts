	...
	switch (pageId) {
		case 'overview': {
			...
			var embedDescription = [
				...
				`${Emojis.ReplyBottom} **Ping:** ${Math.round(
					client.ws.ping
				)}ms`,
			];
			break;
		...
		case 'activity': {
			...
			var embedDescription = [
				...
				`${Emojis.ReplyMiddle} **Log Channel:** ${
					actConf?.logChannel ? `<#${actConf.logChannel}>` : 'None'
				}`,
				`${Emojis.ReplyMiddle} **Daily Active Role:** ${
					actConf?.dailyActiveRole
						? `<@&${actConf.dailyActiveRole}>`
						: 'None'
				}`,
				`${Emojis.ReplyBottom} **Weekly Active Role:** ${
			...
			rows.push(
				...
				new ActionRowBuilder<RoleSelectMenuBuilder>().addComponents(
					client.Pages.generateSelectMenu({
						customId: client.createCustomId({
							customId: RoleSM.customId,
							valueOne: pageId,
							valueTwo: 'dailyActiveRole',
							ownerId: pageOwner.id,
						}),
						placeholder: 'Daily Active Role',
						defaults: actConf?.dailyActiveRole
							? [actConf.dailyActiveRole]
							: [],
						type: 'role',
					})
				...
				new ActionRowBuilder<RoleSelectMenuBuilder>().addComponents(
					client.Pages.generateSelectMenu({
						customId: client.createCustomId({
							customId: RoleSM.customId,
							valueOne: pageId,
							valueTwo: 'weeklyActiveRole',
							ownerId: pageOwner.id,
						}),
						placeholder: 'Weekly Active Role',
						defaults: actConf?.weeklyActiveRole
							? [actConf.weeklyActiveRole]
							: [],
						type: 'role',
					})
